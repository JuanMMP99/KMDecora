...
function renderFooter() {
  document.getElementById('site-footer').innerHTML = `
    ...
    <div class="footer-bottom">
      <span>© 2025 KM Decora · Todos los derechos reservados</span>
...
function renderWelcomeModal() {
  ...
  document.getElementById('welcome-accept-btn').addEventListener('click', () => {
  ...
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) {
      ...
      setTimeout(() => { modal.style.transform = ''; }, 150);
    }
  });
}

/* ── 4. WHATSAPP FAB ───────────────────────────────────── */
function renderWhatsApp(phone = '521234567890', msg = 'Hola KM Decora, quiero hacer un pedido 🎁') {
...
const ProductModal = (() => {
  ...
  function open(productoId) {
    ...
    // Rellenar datos
    document.getElementById('modal-cat').textContent    = categoriaNombre(p.categoria);
    document.getElementById('modal-title').textContent  = p.nombre;
    document.getElementById('modal-price').textContent  = `$${p.precio.toLocaleString()}`;
    document.getElementById('modal-unit').textContent   = `MXN / ${p.unidad}`;
    document.getElementById('modal-anticipacion').textContent = `⏰ Anticipa tu pedido: ${p.anticipacion}`;
    document.getElementById('modal-desc').textContent   = p.descripcionLarga || p.descripcion;
    ...
    document.getElementById('modal-wa-btn').onclick = () => {
      const msg = `Hola KM Decora! Me interesa: *${p.nombre}* ($${p.precio} MXN). ¿Tienes disponibilidad?`;
      window.open(`https://wa.me/521234567890?text=${encodeURIComponent(msg)}`, '_blank');
    };

    // Cotizar
    document.getElementById('modal-cart-btn').onclick = () => {
      Cart.add({ id: p.id, name: p.nombre, price: p.precio, img: currentImgs[0] || '', cat: categoriaNombre(p.categoria) });
      close();
    };
...
const Cart = (() => {
  ...
  function updateCounter() {
    ...
    requestAnimationFrame(() => { c.style.animation = 'cartBounce 0.4s ease'; });
  }
  ...
  function getQuoteText() {
    ...
    const list = items.map(i => `• ${i.name} — $${i.price.toLocaleString()} MXN`).join('\n');
  ...
  function buildPanel() {
    ...
    document.getElementById('cart-wa-btn').addEventListener('click', () => {
      ...
      if (!text) return;
      window.open(`https://wa.me/521234567890?text=${encodeURIComponent(text)}`, '_blank');
    });
...
const Lightbox = (() => {
  ...
  function show() {
    ...
    document.getElementById('lb-caption').textContent = imgs[idx]?.caption || '';
      ...
      <div class="product-card-body">
        ...
        <div class="product-card-footer">
          ...
          <button class="add-btn"
            ...
            onclick="event.stopPropagation(); Cart.add({id:'${p.id}', name:'${p.nombre.replace(/'/g,"\\'")}', price:${p.precio}, img:'${p.fotos[0]}', cat:'${categoriaNombre(p.categoria)}'})">
            +
          </button>
        </div>
      </div>
